...
namespace SVD {
  ...
  namespace XLNX_CTRL {
    namespace UPDDetails {
      ...
      void Unpacker::EventLoop(PayloadBuffer_t &rBuffer) noexcept {
        ...
        while (this->IsRunning()) {
          ...
          if (itBegin != std::end(frame)) {
            ...
            for (itBegin = std::find_if(itEnd, std::end(frame),
                                        &Unpacker::IsMainHeader);
                 itBegin != std::end(frame);
                 itBegin = std::find_if(itEnd, std::end(frame),
                                        &Unpacker::IsMainHeader)) {

              itEnd =
                  std::find_if(itBegin, std::end(frame), &Unpacker::IsTrailer);
              //        std::cout << "got end\n";
      ...
      Sorter::Sorter(PayloadBuffer_t &rBuffer,
          ...
          : m_PiggyBuffer(std::make_unique<FADCPayloadBuffer_t>()),
            ...
            m_IsRunning(std::make_unique<std::atomic<bool>>(true)),
            m_euLogger(logger),
            m_pThread(std::make_unique<std::thread>(&Sorter::eventLoop, this,
                                                    std::ref(rBuffer))) {}
      ...
      void Sorter::sortData(Payload_t &frame, FADCPayload_t &buffer,
        ...
        buffer.resize(m_gMaxBufferSize); // reserve space for worst case, full
                                         // frame contains only one type of data
        ...
        auto it =
            std::copy_if(frame.begin(), frame.end(), buffer.begin(),
