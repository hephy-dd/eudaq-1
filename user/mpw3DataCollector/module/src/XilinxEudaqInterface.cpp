...
bool FADCGbEMerger::operator()(Event_t &rEvent) noexcept {
  if (std::any_of(std::begin(m_Unpackers),
                  std::end(m_Unpackers), [](const auto &rUnpacker) noexcept {
  ...
  if (std::any_of(
          std::begin(rEvent.m_Data),
          std::end(rEvent.m_Data), [eventNr](const auto &rData) noexcept {
