...
auto dummy0 = eudaq::Factory<eudaq::Producer>::Register<
    CaribouProducer, const std::string &, const std::string &>(
    CaribouProducer::m_id_factory);
}

CaribouProducer::CaribouProducer(const std::string name,
                                 const std::string &runcontrol)
    : eudaq::Producer(name, runcontrol), m_ev(0), m_exit_of_run(false) {
  // Add cout as the default logging stream
  Log::addStream(std::cout);

  // Separate the device name from the device identifier, the latter is only
  // relevant for EUDAQ:
...
CaribouProducer::~CaribouProducer() { delete manager_; }
...
void CaribouProducer::DoConfigure() {
  ...
  if (config->Has("voltage_key") || config->Has("voltage_value")) {
    ...
    device_->setVoltage(key, value, 3.0);
    EUDAQ_USER("Setting " + key + " = " + std::to_string(value));
  }
